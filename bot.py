...
# Public IP/hostname used in persistent SSH commands. Set PUBLIC_IP in the VPS host environment.
PUBLIC_IP = os.getenv("PUBLIC_IP", "YOUR_SERVER_IP")
RGB_VIDEO_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "primecloud_rgb.mp4")
...
def command_prefix_name(name: str) -> str:
    return f"1{name}"

def valid_custom_command_name(name: str) -> bool:
    return bool(re.fullmatch(r"[a-zA-Z0-9_-]{1,32}", name))

def is_reserved_command(name: str) -> bool:
    return bot.get_command(name) is not None

async def send_rgb_video(ctx, embed=None, *, content=None, view=None):
    if os.path.isfile(RGB_VIDEO_PATH):
        try:
            file = discord.File(RGB_VIDEO_PATH, filename="primecloud_rgb.mp4")
...
async def dm_rgb_video(user, embed):
...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30):
    ...
    - Uses persistent OpenSSH with a mapped host port.
    - Installs OpenSSH server and sets a stable root password.
    """
    ...
    # Run container with a persistent host SSH port.
    # The disk limit uses Docker's storage-opt when supported by the host storage driver.
    storage_opt = f"--storage-opt size={disk_gb}G "
    run_cmd = (
        f"docker run -d "
    ...
    try:
        ...
        # Some Docker storage drivers do not support --storage-opt size.
        logger.warning(f"Storage limit unsupported; retrying without storage-opt: {storage_error}")
        run_cmd = (
            f"docker run -d "
            f"--name {container_name} "
            f"--memory={ram_mb}m "
    ...
    # Install persistent OpenSSH server and configure a stable root password.
    setup_script = (
        "apt-get update -qq && "
        "DEBIAN_FRONTEND=noninteractive apt-get install -y openssh-server curl -qq && "
        "mkdir -p /var/run/sshd && "
        "grep -q '^PermitRootLogin' /etc/ssh/sshd_config || echo 'PermitRootLogin yes' >> /etc/ssh/sshd_config && "
        "grep -q '^PasswordAuthentication' /etc/ssh/sshd_config || echo 'PasswordAuthentication yes' >> /etc/ssh/sshd_config && "
        f"echo 'root:{password}' | chpasswd && "
        "pkill sshd 2>/dev/null || true; /usr/sbin/sshd"
    )
...
def get_ssh_command(vps):
    """Return the persistent SSH command for a VPS. Password is not regenerated."""
    port = int(vps.get("ssh_port", SSH_PORT_START))
    return f"ssh root@{PUBLIC_IP} -p {port}"
...
async def on_ready():
    ...
    await register_saved_custom_commands()
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            class ConfirmView(discord.ui.View):
                ...
                async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        ...
                        ssh_port = int(self.vps.get("ssh_port") or get_next_ssh_port())
                        ram_mb = int(original_ram.replace("GB", "")) * 1024
                        # Keep the same password and SSH port across reinstall.
                        existing_password = self.vps.get("ssh_password") or generate_password()
                        await create_docker_container(
                            self.container_name, ram_mb, original_cpu, ssh_port,
                            existing_password, disk_gb=original_disk
                        )
                        self.vps["status"] = "running"
                        self.vps["ssh_port"] = ssh_port
                        self.vps["ssh_password"] = existing_password
            ...
            try:
                ...
                ssh_cmd = get_ssh_command(vps)
                ssh_embed = create_embed(
                    "🔑 PrimeCloud- SSH Access",
                    f"SSH connection for VPS `{container_name}` (VPS ID `#{self.selected_index + 1}`):",
                    0x00ff88
                )
                ssh_embed.add_field(
                    name="⌯⌲ Command",
                ...
                ssh_embed.add_field(
                    name="⌯⌲ Security",
                    value="This SSH access is persistent. Selecting SSH again does not generate a new password.",
                    inline=False
                )
                ssh_embed.add_field(name="Password", value=f"```{ssh_password}```", inline=False)

                try:
                    await dm_rgb_video(interaction.user, ssh_embed)
                    await interaction.followup.send(
                        embed=create_success_embed("SSH Sent", "Check your DMs for persistent SSH credentials!"),
                        ephemeral=True)
...
async def create_vps(ctx, ram: int, cpu: int, disk: int, user: discord.Member):
    """Create a custom VPS — 1create <ram_GB> <cpu_cores> <disk_GB> @user"""
    if ram <= 0 or cpu <= 0 or disk <= 0:
        await ctx.send(embed=create_error_embed(
            "Invalid Specs",
            "RAM, CPU and Disk must be positive integers.\n"
            "Usage: `1create <ram_GB> <cpu_cores> <disk_GB> @user`"
        ))
        return
    ...
    creating_embed.add_field(name="Resources", value=f"**RAM:** {ram}GB\n**CPU:** {cpu}\n**Storage:** {disk}GB", inline=False)
    await send_rgb_video(ctx, creating_embed)
    ...
    try:
        ...
        # Channel success: intentionally NO RGB video.
        ...
        success_embed.set_footer(text=f"PrimeCloud • {created_at}")
        await ctx.send(embed=success_embed)

        # DM success + persistent SSH access + RGB reaction video.
        dm_embed = create_success_embed("Prime - VPS Created!")
        dm_embed.description = (
            "Your VPS has been successfully deployed by an admin!\n\n"
            "⌯⌲ **VPS Details**\n\n"
            f"**VPS ID:** `#{vps_count}`\n"
            f"**Container Name:** `{container_name}`\n"
            f"**Configuration:** {ram}GB RAM / {cpu} CPU / {disk}GB Disk\n"
            f"**Status:** running\n"
            "**OS:** ubuntu:22.04\n"
            ...
            "Powered by PrimeCloud • Premium VPS Management"
        )
        await dm_rgb_video(user, dm_embed)

        ssh_embed = create_embed(
            "🔑 PrimeCloud- SSH Access",
            f"SSH connection for VPS `{container_name}` (VPS ID `#{vps_count}`):",
            0x00ff88
        )
        ssh_embed.add_field(
            name="⌯⌲ Command",
            value=f"```{get_ssh_command(vps_info)}```",
            inline=False
        )
        ssh_embed.add_field(
            name="⌯⌲ Security",
            value="This SSH access is persistent for this VPS. The password is not regenerated when you select SSH.",
            inline=False
        )
        ssh_embed.add_field(name="Password", value=f"```{password}```", inline=False)
        await dm_rgb_video(user, ssh_embed)
...
async def manage_vps(ctx, user: discord.Member = None):
    ...
    if user:
        ...
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            embed = create_embed("No VPS Found", "You don't have any VPS. Use `1buywc` to purchase one.", 0xff3366)
            embed.add_field(name="Quick Actions", value="• `1plans` - View plans\n• `1buywc <plan> <processor>` - Purchase VPS", inline=False)
            await ctx.send(embed=embed)
            return
        view = ManageView(user_id, vps_list)
        await send_rgb_video(ctx, view.initial_embed, view=view)
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    try:
        await create_docker_container(container_name, ram_mb, cpu_str, ssh_port, password)
        ...
        try:
            ssh_cmd = get_ssh_command(vps_info)

            dm_embed = create_embed("🎉 Your VPS is Ready!", "Connect now using the persistent SSH command below.", color=0x5865F2)
...
def build_help_pages(is_user_admin, is_user_main_admin):
    ...
    extras_embed.add_field(name="ℹ️ Info", value=(
        ...
        "`1myvps` — Your VPS list\n"
        "`1myinfo` — Your personal dashboard\n"
        "`1custom_command <command> <response>` — Add a custom command (admin)"
    ), inline=False)
    ...
    if is_user_admin:
        ...
        admin_embed.add_field(name="🖥️ VPS Control", value=(
            "`1create <ram_GB> <cpu_cores> <disk_GB> @user` — Create custom Docker VPS\n"
...
@bot.command(name='custom_command', aliases=['coustem_command'])
@is_admin()
async def add_custom_command(ctx, command_name: str, *, response: str):
    """Add a custom text command — 1custom_command <command> <response>"""
    command_name = command_name.lower().strip()
    if not valid_custom_command_name(command_name):
        await ctx.send(embed=create_error_embed(
            "Invalid Command",
            "Command name must be 1-32 characters and use only letters, numbers, `_` or `-`."
        ))
        return
    if is_reserved_command(command_name) or command_name in {"custom_command", "coustem_command"}:
        await ctx.send(embed=create_error_embed("Command Exists", f"`1{command_name}` is already reserved."))
        return
    if not response.strip():
        await ctx.send(embed=create_error_embed("Missing Response", "Add the response after the command name."))
        return

    old = custom_commands.get(command_name)
    custom_commands[command_name] = response
    save_custom_commands()

    # Remove an old runtime command if it was replaced.
    existing = bot.get_command(command_name)
    if existing:
        bot.remove_command(existing)

    async def custom_callback(message_ctx):
        await message_ctx.send(custom_commands.get(command_name, response))

    cmd = commands.Command(custom_callback, name=command_name, help="Custom command")
    bot.add_command(cmd)

    action = "updated" if old is not None else "added"
    await ctx.send(embed=create_success_embed(
        "Custom Command Saved",
        f"Custom command `1{command_name}` {action} successfully.\n\n"
        f"**Response:**\n{response}"
    ))
...
async def show_help(ctx):
    ...
    # The help UI is intentionally available through the 1 prefix only.
    ...
    await send_rgb_video(ctx, pages["user"], view=HelpView())  # Video accompanies 1help
...
async def my_vps(ctx):
    ...
    vps_list = vps_data.get(user_id, [])
    if not vps_list:
        embed = create_error_embed("No VPS Found", "You don't have any VPS yet.")
    else:
        embed = create_embed("🖥️ My VPS", f"{ctx.author.mention}, your VPS list:")
        for i, v in enumerate(vps_list, 1):
            embed.add_field(
                name=f"VPS #{i}",
                value=(
                    f"**Container:** `{v['container_name']}`\n"
                    f"**RAM:** {v.get('ram', '?')} | **CPU:** {v.get('cpu', '?')} Core(s) | **Storage:** {v.get('storage', '?')}\n"
                    f"**Status:** `{v.get('status', 'unknown').upper()}`"
                ),
                inline=False
            )
    await send_rgb_video(ctx, embed)
...
async def register_saved_custom_commands():
    """Register custom commands saved on disk after built-in commands exist."""
    for command_name, response in custom_commands.items():
        if not valid_custom_command_name(command_name):
            continue
        if bot.get_command(command_name) is not None:
            continue

        async def custom_callback(ctx, _name=command_name, _response=response):
            await ctx.send(custom_commands.get(_name, _response))

        bot.add_command(commands.Command(custom_callback, name=command_name, help="Custom command"))


# ─── Bot token ────────────────────────────────────────────────────────────────
# Paste your Discord bot token between the quotes below.
DISCORD_TOKEN = "primecloud_token_here"

if __name__ == "__main__":
    token = DISCORD_TOKEN.strip()
    if token == "primecloud_token_here" or not token:
        raise SystemExit("Set DISCORD_TOKEN in bot.py: replace primecloud_token_here with your bot token.")
    bot.run(token)