...
def create_embed(title, description="", color=0x1a1a1a):
    ...
    embed.set_thumbnail(url="https://www.image2url.com/r2/default/images/1784800892312-1401f571-cc6d-4d12-8993-1ad7f7690bd4.png")
    ...
    if action == "config":
        ...
        if len(tokens) >= 5 and tokens[1] == "set":
            ...
            if key == "limits.memory":
                ...
                return ["docker", "update", "--memory", value, name], None
...
async def execute_lxc(container_name: str, command: str, timeout=120, node_id: Optional[int] = None):
    ...
    if special:
        ...
        if kind == "proxy_add":
            ...
            _start_proxy(name, proto, host_port, vps_port)
            return True

        if kind == "proxy_remove":
            _, name, proto, host_port = special
            _stop_proxy(_proxy_pid_path(name, proto, host_port))
            return True

        if kind == "copy_create":
            image, dest = special[1], special[2]
            _run_sync(argv, timeout)
            _run_sync([
        ...
        if kind == "restore":
            ...
            # Recreate the container from a previously committed snapshot image.
            _run_sync(["docker", "rm", "-f", name], timeout)
            _run_sync([
    ...
    try:
        result = _run_sync(argv, timeout=timeout) if argv[0] != "true" else True

        # Docker can assign a new container IP after restart/reinstall.
        # Rebuild saved host->container proxies after every successful start.
