...
BOT_NAME = os.getenv('BOT_NAME', 'MightVM')
PREFIX = os.getenv('PREFIX', '!')
YOUR_SERVER_IP = os.getenv('YOUR_SERVER_IP', '127.0.0.1')
MAIN_ADMIN_ID = int(os.getenv('MAIN_ADMIN_ID', '1506264876878794793'))
VPS_USER_ROLE_ID = int(os.getenv('VPS_USER_ROLE_ID', '1210291131301101618'))
DEFAULT_STORAGE_POOL = os.getenv('DEFAULT_STORAGE_POOL', 'default')
BOT_VERSION = os.getenv('BOT_VERSION', '7.0-PRO')
BOT_DEVELOPER = os.getenv('BOT_DEVELOPER', 'MightExE')
...
def allocate_global_vps_number() -> int:
    """Allocate a never-reused VPS number from SQLite."""
    conn = get_db()
    try:
        cur = conn.cursor()
        cur.execute("BEGIN IMMEDIATE")
        cur.execute("SELECT value FROM settings WHERE key = 'next_vps_number'")
        row = cur.fetchone()
        number = int(row[0]) if row else 1
        cur.execute(
            "INSERT OR REPLACE INTO settings(key, value) VALUES('next_vps_number', ?)",
            (str(number + 1),)
        )
        conn.commit()
        return number
...
def docker_preflight():
    """Fail early with a useful message instead of partially creating VPSs."""
    if shutil.which("docker") is None:
        raise RuntimeError("Docker CLI is not installed or not in PATH.")
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "Docker daemon is unavailable. Start Docker and ensure the bot user "
            "can run `docker ps`."
        )

try:
    docker_preflight()
    logger.info("Docker preflight: OK")
except Exception as _docker_error:
    logger.warning("Docker preflight warning: %s", _docker_error)
...
def create_embed(title, description="", color=0x1a1a1a):
    ...
    LOGO_URL = "https://cdn.discordapp.com/attachments/1532141257328169111/1537428457175261284/file_00000000986081fa9257ed777ad5f745.png?ex=6a7f0154&is=6a7dafd4&hm=fac514b08603125e210367b00ace7dd8d83362ce2576a1418d1be9ebd7690aa0&"
...
# Docker backend command execution with multi-node support
# ---------------------------------------------------------------------------
# Docker backend compatibility layer
# ---------------------------------------------------------------------------
# The rest of this bot still calls execute_lxc() so the Discord command/UI
# layer can remain compatible. The function translates the old LXC-like
# operations into Docker operations.
#
# Docker does NOT provide a universal per-container filesystem quota. The
# requested disk value is therefore stored as metadata/labels, while RAM/CPU
# are enforced by Docker. Port forwarding is implemented with host-side socat.
# ---------------------------------------------------------------------------

PROXY_DIR = "/var/run/primecloud-proxy"

def _docker_image(image: str) -> str:
    image = image.strip()
    if image.startswith("images:"):
        image = image.replace("images:", "", 1)
    # LXC's ubuntu:22.04 is already a Docker-compatible image name.
    return image

def _run_sync(argv, timeout=120):
    proc = subprocess.run(
        argv, capture_output=True, text=True, timeout=timeout
    )
    if proc.returncode != 0:
        raise Exception((proc.stderr or proc.stdout or "Docker command failed").strip())
...
def _proxy_pid_path(container: str, proto: str, host_port: int) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]", "_", container)
    return os.path.join(PROXY_DIR, f"{safe}_{proto}_{host_port}.pid")

def _start_proxy(container: str, proto: str, host_port: int, vps_port: int):
    os.makedirs(PROXY_DIR, exist_ok=True)
    ip = _container_ip(container)
    if not ip:
        raise Exception(f"Could not determine Docker IP for {container}")

    pid_file = _proxy_pid_path(container, proto, host_port)

    # Remove an old proxy for the same port first.
    ...
    if proto == "tcp":
        ...
        argv = [
            "socat", f"UDP-LISTEN:{host_port},fork,reuseaddr",
            f"UDP:{ip}:{vps_port}"
        ]
...
def _stop_container_proxies(container: str):
    if not os.path.isdir(PROXY_DIR):
        return
    safe = re.sub(r"[^A-Za-z0-9_.-]", "_", container)
    for name in os.listdir(PROXY_DIR):
        if name.startswith(safe + "_"):
            _stop_proxy(os.path.join(PROXY_DIR, name))

def _translate_compat_command(container_name: str, command: str):
    ...
    if action == "init":
        if len(tokens) < 3:
            raise Exception("Usage: init IMAGE CONTAINER")
        image = _docker_image(tokens[1])
        name = tokens[2]

        # Never reuse a generic/fixed container name.  The Discord layer
        # allocates names such as primecloud-1, primecloud-2, ...
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", name):
            raise Exception(f"Invalid Docker container name: {name}")

        # Fail cleanly instead of accidentally deleting an existing VPS.
        ...
        if exists:
            raise Exception(
                f"Container {name!r} already exists. "
                "Refusing to overwrite it; use the existing VPS or delete it explicitly."
            )
        ...
        return [
            ...
            "--privileged",
            "--init",
            "--label", "primecloud.managed=true",
            "--label", f"primecloud.container_id={name}",
            image, "sleep", "infinity"
        ], None

    if action == "pull":
        if len(tokens) < 2:
            raise Exception("Usage: pull IMAGE")
        return ["docker", "pull", _docker_image(tokens[1])], None

    if action == "start":
        name = tokens[1] if len(tokens) > 1 else container_name
        return ["docker", "start", name], None

    if action == "stop":
        name = tokens[1] if len(tokens) > 1 else container_name
        return ["docker", "stop", name], None

    if action == "restart":
        name = tokens[1] if len(tokens) > 1 else container_name
        return ["docker", "restart", name], None

    if action == "delete":
        name = tokens[1] if len(tokens) > 1 else container_name
        _stop_container_proxies(name)
        return ["docker", "rm", "-f", name], None

    if action == "rename":
        if len(tokens) < 3:
            raise Exception("Usage: rename OLD NEW")
        return ["docker", "rename", tokens[1], tokens[2]], None

    if action == "exec":
        if len(tokens) < 3:
            raise Exception("Usage: exec CONTAINER -- COMMAND")
        name = tokens[1]
        try:
            sep = tokens.index("--")
            inner = tokens[sep + 1:]
        ...
        if not inner:
            inner = ["sh"]
        return ["docker", "exec", name] + inner, None

    if action == "list":
        return [
            "docker", "ps", "-a",
            "--format",
            "table {{.Names}}\t{{.Status}}\t{{.Image}}\t{{.RunningFor}}"
        ], None

    if action == "config":
        # config set CONTAINER KEY VALUE
        if len(tokens) >= 5 and tokens[1] == "set":
            name = tokens[2]
            key = tokens[3]
            value = tokens[4]

            if key == "limits.memory":
                value = value.replace("MB", "m").replace("GB", "g")
                return [
                    "docker", "update",
                    "--memory", value,
                    "--memory-swap", value,
            ...
            if key == "limits.cpu":
                return ["docker", "update", "--cpus", value, name], None

            # Docker cannot apply LXC-specific nesting/raw config after create.
            if key.startswith("security.") or key.startswith("linux.") or key == "raw.lxc":
                return ["docker", "inspect", name], "ignored"

        # config device ... is handled separately below.
        if len(tokens) >= 4 and tokens[1] == "device":
            name = tokens[2]
            operation = tokens[3]

            # config device add NAME tcp_proxy_PORT proxy listen=... connect=...
            if operation == "add" and len(tokens) >= 5:
                device_name = tokens[4]
                if device_name.startswith("tcp_proxy_") or device_name.startswith("udp_proxy_"):
                    host_port = int(device_name.rsplit("_", 1)[1])
                    proto = "tcp" if device_name.startswith("tcp_") else "udp"
                    listen = next((x for x in tokens if x.startswith("listen=")), "")
                    connect = next((x for x in tokens if x.startswith("connect=")), "")
                    m = re.search(r"connect=\w+:127\.0\.0\.1:(\d+)", connect)
                    if not m:
                        raise Exception("Could not parse proxy target port")
                    vps_port = int(m.group(1))
                    return ["true"], ("proxy_add", name, proto, host_port, vps_port)

            if operation == "remove" and len(tokens) >= 5:
                device_name = tokens[4]
                if device_name.startswith("tcp_proxy_") or device_name.startswith("udp_proxy_"):
                    host_port = int(device_name.rsplit("_", 1)[1])
                    proto = "tcp" if device_name.startswith("tcp_") else "udp"
                    return ["true"], ("proxy_remove", name, proto, host_port)

            # root size / NIC / rate limits are not portable to Docker.
            return ["docker", "inspect", name], "ignored"

        return ["docker", "inspect", tokens[2] if len(tokens) > 2 else container_name], "ignored"

    if action == "copy":
        if len(tokens) < 3:
            raise Exception("Usage: copy SOURCE DEST")
        source, dest = tokens[1], tokens[2]
        image = f"primecloud-copy-{dest}:latest"
        return ["docker", "commit", source, image], ("copy_create", image, dest)

    if action == "snapshot":
        if len(tokens) >= 3 and tokens[1] != "list":
            name, snap = tokens[1], tokens[2]
            image = f"primecloud-snapshot-{name}-{snap}:latest"
            return ["docker", "commit", name, image], ("snapshot_create", image)

        if len(tokens) >= 2 and tokens[1] == "list":
            result = subprocess.run(
                ["docker", "images", "--format", "{{.Repository}}:{{.Tag}}",
                 "--filter", "reference=primecloud-snapshot-*"],
                capture_output=True, text=True
            )
            return ["printf", "%s", result.stdout.strip()], None

    if action == "restore":
        if len(tokens) < 3:
            raise Exception("Usage: restore CONTAINER SNAPSHOT")
        name, snap = tokens[1], tokens[2]
        image = f"primecloud-snapshot-{name}-{snap}:latest"
        return ["docker", "image", "inspect", image], ("restore", name, image)

    if action == "profile" and len(tokens) > 1 and tokens[1] == "list":
        return ["docker", "info"], None

    if action == "storage" and len(tokens) > 1 and tokens[1] == "list":
        return ["docker", "system", "df"], None

    raise Exception(f"Unsupported LXC operation in Docker backend: {command}")

async def execute_lxc(container_name: str, command: str, timeout=120, node_id: Optional[int] = None):
    """Compatibility name retained for the existing bot UI; backend is Docker."""
    if node_id is None:
        node_id = find_node_id_for_container(container_name)
    ...
    if not node['is_local']:
        # Remote nodes use the same generic /api/execute endpoint.
        # Translate to a shell command string for the remote Docker host.
        try:
            argv, special = _translate_compat_command(container_name, command)
            if special in ("ignored", None):
                remote_command = " ".join(shlex.quote(x) for x in argv)
            else:
                if special and special[0] == "proxy_add":
                    _, name, proto, hp, vp = special
                    remote_command = (
                        f"socat {proto.upper()}-LISTEN:{hp},fork,reuseaddr "
                        f"{proto.upper()}:$(docker inspect -f '{{{{range .NetworkSettings.Networks}}}}{{{{.IPAddress}}}}{{{{end}}}}}}' "
                        f"{shlex.quote(name)}):{vp}"
                    )
                elif special and special[0] == "proxy_remove":
                    remote_command = "true"
                else:
                    remote_command = "true"
    ...
    # Translation can perform a quick Docker inspect; keep it off the Discord event loop.
    argv, special = await asyncio.to_thread(_translate_compat_command, container_name, command)

    if special == "ignored":
        logger.info("Docker backend ignored non-portable LXC setting: %s", command)
        return True
    ...
    if special:
        ...
        if kind == "proxy_add":
            _, name, proto, host_port, vps_port = special
            if shutil.which("socat") is None:
                raise Exception(
                    "socat is required for dynamic port forwarding. "
                    "Install it with: apt-get update && apt-get install -y socat"
                )
            await asyncio.to_thread(_start_proxy, name, proto, host_port, vps_port)
            return True

        if kind == "proxy_remove":
            _, name, proto, host_port = special
            await asyncio.to_thread(_stop_proxy, _proxy_pid_path(name, proto, host_port))
            return True

        if kind == "copy_create":
            image, dest = special[1], special[2]
            await asyncio.to_thread(_run_sync, argv, timeout)
            await asyncio.to_thread(_run_sync, [
                "docker", "create", "--name", dest, "--hostname", dest,
                "--privileged", image, "sleep", "infinity"
            ], timeout)
            return True
        ...
        if kind == "restore":
            _, name, image = special
            await asyncio.to_thread(_run_sync, ["docker", "rm", "-f", name], timeout)
            await asyncio.to_thread(_run_sync, [
    ...
    try:
        result = True if argv[0] == "true" else await asyncio.to_thread(_run_sync, argv, timeout)

        if argv[:2] == ["docker", "start"]:
            try:
                await recreate_port_forwards(argv[2] if len(argv) > 2 else container_name)
            except Exception as proxy_error:
                logger.warning("Could not recreate saved port forwards: %s", proxy_error)

        return result
    except subprocess.TimeoutExpired:
        raise asyncio.TimeoutError(f"Docker command timed out after {timeout} seconds")
    except Exception as e:
        raise Exception(f"Docker backend command failed: {command}\n{e}")

# Docker replacement for the old LXC-specific configuration routine.
async def apply_lxc_config(container_name: str, node_id: int):
    # Containers are created privileged because the old bot requested
    # nesting/FUSE/kernel-module access. Docker cannot safely translate all
    # raw.lxc options; privileged mode is the closest portable equivalent.
    logger.info("Docker backend: privileged mode already applied to %s", container_name)
    return True

async def apply_internal_permissions(container_name: str, node_id: int):
    # Keep the function name/API expected by the existing bot.
    try:
        await execute_lxc(
            container_name,
            f"exec {container_name} -- chmod 1777 /tmp",
            node_id=node_id
        )
    except Exception:
        pass
...
async def get_container_status_local(container_name: str):
    try:
        out = _run_sync(["docker", "inspect", "-f", "{{.State.Status}}", container_name])
...
async def get_container_cpu_pct_local(container_name: str):
    try:
        ...
        value = stdout.decode().strip().replace("%", "")
        return float(value) if value else 0.0
    except Exception as e:
        logger.error(f"Error getting Docker CPU for {container_name}: {e}")
        return 0.0
...
async def get_container_ram_local(container_name: str):
    try:
        ...
        usage, pct = raw.split("|", 1)
        pct_val = float(pct.replace("%", "").strip() or 0)
        # Docker returns e.g. "123MiB / 1GiB". Convert the displayed
        # used/total values approximately to MB for the existing UI.
        def to_mb(s):
            m = re.match(r"([\d.]+)\s*([KMGTP]i?B)", s.strip())
            if not m:
                return 0
            n, unit = float(m.group(1)), m.group(2).lower()
            factors = {"kb": 1/1024, "kib": 1/1024, "mb": 1,
                       "mib": 1, "gb": 1024, "gib": 1024,
                       "tb": 1024*1024, "tib": 1024*1024,
                       "b": 1/(1024*1024)}
            return int(n * factors.get(unit, 1))
        used_s, total_s = usage.split("/", 1)
        return {'used': to_mb(used_s), 'total': to_mb(total_s), 'pct': pct_val}
    except Exception as e:
        logger.error(f"Error getting Docker RAM for {container_name}: {e}")
        return {'used': 0, 'total': 0, 'pct': 0.0}
...
def get_default_storage_pool():
    # Docker does not have LXC storage pools. Keep the setting for
    # compatibility with the existing database/UI.
    return "local"
...
class OSSelectView(discord.ui.View):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        # Global, never-reused sequential IDs prevent Docker name collisions
        # even when many users create/delete VPSs.
        try:
            vps_number = allocate_global_vps_number()
            safe_bot_name = re.sub(r"[^a-z0-9-]+", "-", BOT_NAME.lower()).strip("-") or "primecloud"
            container_name = f"{safe_bot_name}-{vps_number}"
            ram_mb = self.ram * 1024
            # Docker does not auto-pull images. Pull explicitly before create.
            await execute_lxc(container_name, f"pull {os_version}", timeout=300, node_id=self.node_id)
            await execute_lxc(container_name, f"init {os_version} {container_name} -s {DEFAULT_STORAGE_POOL}", timeout=120, node_id=self.node_id)
            await interaction.edit_original_response(embed=create_info_embed("Creating VPS", f"Image ready. Applying **{self.ram}GB RAM / {self.cpu} CPU / {self.disk}GB** limits..."), view=self)
            ...
            add_field(success_embed, "VPS ID", f"#{vps_number}", True)
...
class ManageView(discord.ui.View):
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'start':
            ...
            try:
                # Install both OpenSSH and tmate. tmate provides a temporary
                # public SSH relay, while openssh-server gives the container a
                # normal SSH daemon for direct/forwarded access.
                try:
                    await execute_lxc(container_name, f"exec {container_name} -- sh -c 'command -v tmate >/dev/null 2>&1 && command -v sshd >/dev/null 2>&1'", node_id=node_id)
                except:
                    await interaction.followup.send(embed=create_info_embed("Installing SSH", "Installing OpenSSH + tmate..."), ephemeral=True)
                    await execute_lxc(container_name, f"exec {container_name} -- env DEBIAN_FRONTEND=noninteractive apt-get update -y", node_id=node_id)
                    await execute_lxc(container_name, f"exec {container_name} -- env DEBIAN_FRONTEND=noninteractive apt-get install -y openssh-server tmate sudo ca-certificates", node_id=node_id)
                    await execute_lxc(container_name, f"exec {container_name} -- mkdir -p /run/sshd", node_id=node_id)
                    ssh_config_cmd = (
                    ...
                    await execute_lxc(
                        container_name,
                        f"exec {container_name} -- sh -c {shlex.quote(ssh_config_cmd)}",
                        node_id=node_id
                    )
                    await interaction.followup.send(embed=create_success_embed("Installed", "OpenSSH + tmate installed."), ephemeral=True)

                session_name = f"{BOT_NAME.lower()}-session-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
                socket_path = f"/tmp/{session_name}.sock"
                await execute_lxc(container_name, f"exec {container_name} -- sh -c 'mkdir -p /run/sshd; /usr/sbin/sshd 2>/dev/null || true; tmate -S {socket_path} new-session -d'", node_id=node_id)
                await asyncio.sleep(4)
                ssh_output = await execute_lxc(container_name, f"exec {container_name} -- tmate -S {socket_path} display -p '#{{tmate_ssh}}'", node_id=node_id)
            ...
            try:
                ...
                for node in nodes:
                    if node['is_local']:
                        ...
                        url = f"{node['url']}/api/execute"
                        data = {"command": "docker stop $(docker ps -q)"}
                        params = {"api_key": node["api_key"]}
                        response = requests.post(url, json=data, params=params)
                        if response.status_code != 200:
        ...
        self.command_categories = {
            ...
            "nodes": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}lxc-list [node_id]", "List Docker containers on node (Admin only)")
                ],
                "admin_only": True
            },
            "bot": {
